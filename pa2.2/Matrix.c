...
int equals(Matrix A, Matrix B)
{
	...
	for (int i = 1; i <= size(A); i++)
		if (A->rows[i] != NULL && B->rows[i] != NULL)
		{
			if (length(A->rows[i]) != length(B->rows[i]))
				return 0;
			else if (length(A->rows[i]) > 0 && length(B->rows[i]) > 0)
			{
				moveFront(A->rows[i]);
				moveFront(B->rows[i]);
...
void changeEntry(Matrix M, int i, int j, double x)
{
	...
	else if (x == 0 && M->rows[i] != NULL && length(M->rows[i]) > 0 && getAt(M->rows[i], j) != NULL)
	{
		// printf("%s: [%d,%d] -%.1f\n", "deleting", i, j, x); //[3,3] - 0 , not deleting
		for (moveFront(M->rows[i]); index(M->rows[i]) != j && index(M->rows[i]) != -1; moveNext(M->rows[i]))
			;
		delete (M->rows[i]);
	...
	// else if (x == 0 && M->rows[i] != NULL && length(M->rows[i]) == 1 && front(M->rows[i]) != NULL)
	// {
	// 	printf("%s: [%d,%d] -%.1f\n", "missing", i, j, x);
	// 	moveFront(M->rows[i]);
	// 	delete (M->rows[i]);
	...
	// }
	//[3,3] - 0 , not deleting
	// else if (x == 0 && M->rows[i] != NULL && length(M->rows[i]) == 1)
	// {
	// 	deleteBack(M->rows[i]);
	// 	freeList(&M->rows[i]);
	// 	M->rows[i] = NULL;
	// }
	// else if (x == 0)
	// 	printf("[%d,%d]: %d %d %d\n", i, j, M->rows[i] != NULL, length(M->rows[i]) > 0, getAt(M->rows[i], j) != NULL);
}
